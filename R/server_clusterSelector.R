...
.buildClusterSelectorServer <- function(sce,
  ...
  function(input, output, session) {
    ...
    PercentBarPlot <- shiny::reactive({
      ...
      grpInp <- groupsInput()
      PercentBarPlotFunc(
        sce, relativeToCol, clusterPatientTable, rs,
    ...
    vlnPlot <- shiny::reactive({
      ...
      ggplot2::ggplot(data, ggplot2::aes(factor(marker), expr, fill = marker)) +
        ...
        ggplot2::scale_y_continuous(
          ...
          labels = function(x) {
            c(
              rep(x = "", times = length(x) - 2),
              x[length(x) - 1], ""
            )
          }
        ) +
        ggplot2::facet_grid(
          rows = ggplot2::vars(grpName),
    ...
    if (isTRUE(verbose)) {
      ...
      shiny::observe({
        ...
        dims <- activeDims()
        # cat(sprintf(
        #   "[CySA] SOM base plot rebuilt (%s vs %s) at %s\n",
        #   dims[1], dims[2], format(Sys.time(), "%H:%M:%S")
        # ), file = stderr())
      }) %>% shiny::bindCache(activeDims(), input$somColorVar, input$somSizeVar)
    }
    ...
    .registerClusterSelectorOutputs(
      ...
      colsUsed, rnSCE, nPlots, df, dfPlot,
      env = env_local, verbose
    )
  }
  # nocov end
}
