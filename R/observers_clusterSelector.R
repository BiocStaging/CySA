...
.registerClusterSelectorObservers <- function(input, output, session,
  ...
  shiny::observeEvent(input$rmGroups, {
    ...
    if (is.null(rmGroups) || length(rmGroups) == 0L) {
      return(NULL)
    }
    outputList <- rv$outputList
  ...
  lapply(seq_len(nPlots), function(i) {
    shiny::observeEvent(
      suppressWarnings(
        .safeEventData(
          verbose = verbose, "plotly_selected",
